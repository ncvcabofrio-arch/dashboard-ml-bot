...
from puxador import (sb, ml_get, lista_refresh_tokens, renovar_token,
                     enriquecer_frete, enriquecer_repasse, enriquecer_local,
                     tipo_anuncio)
...
DATA_INI = os.environ.get("DATA_INI", "").strip()  # opcional YYYY-MM-DD
DATA_FIM = os.environ.get("DATA_FIM", "").strip()  # opcional YYYY-MM-DD (vazio = hoje)
# MODO = "tudo" (padrao) puxa pedidos + enriquece.
# MODO = "enriquecer" PULA a puxada de pedidos e só termina frete/repasse/custo
#        (retoma de onde parou, sem refazer a parte que já está no banco).
MODO = os.environ.get("MODO", "tudo").strip().lower()
...
def janelas():
    ini = _pd(DATA_INI) if DATA_INI else date(ANO_INI, MES_INI, 1)
    fim = _pd(DATA_FIM) if DATA_FIM else datetime.now(timezone.utc).date()
    out, d = [], ini
    while d <= fim:
        prox = d + timedelta(days=PASSO)
        ate_dt = min(prox, fim + timedelta(days=1))
        de = d.strftime("%Y-%m-%dT00:00:00.000-03:00")
        ate = ate_dt.strftime("%Y-%m-%dT00:00:00.000-03:00")
